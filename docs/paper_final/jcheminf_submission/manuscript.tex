...
\textbf{Background.} Nuclear magnetic resonance (NMR) chemical-shift predictors
...
\textbf{Results.} We treat the HSQC peak list as an unordered multiset in
...
\textbf{Conclusions.} Unassigned 2-D HSQC peak lists carry enough supervisory
...
\texttt{https://github.com/dongzhaohe321418-lab/nmr-ssl}.
...
\emph{Experimental details} covers dataset construction, model architecture,
training protocol, split-conformal calibration, and the
structure-verification procedure. \emph{Results} reports the main label
...
\emph{Discussion} covers honest limitations and the path to real literature
HSQC data. \emph{Conclusions} summarizes the headline findings. The
...
\emph{Results: Hyperparameter sensitivity}). We report both variants; the axis-aligned version is what
...
\item
  \textbf{Absolute accuracy below published SOTA.} NMRNet {[}1{]} achieves
...
construction filters in \texttt{src/nmr2d/data\_2d.py} applied to seed-0
random split indices from \texttt{experiments/run\_2d\_experiment.py}. Trained
...
\texttt{https://github.com/dongzhaohe321418-lab/nmr-ssl} under the MIT license
...
audit runs via \texttt{python3\ experiments/run\_h\_zero\_ablation.py}; the
Bonferroni-corrected conformal calibration is computed by
\texttt{python3\ experiments/compute\_bonferroni\_conformal.py}. Operating
system, Python version, and dependency versions are pinned in the
repository's \texttt{pyproject.toml}.
...
\def\labelenumi{\arabic{enumi}.}
\item
  Xu F, Guo W, Wang F, Yao L, Wang H, Tang F, et al.~(2025) Toward a
...
\item
  Dong Z (2026) A sort-match theorem for unassigned-set regression
...
\item
  Hardy GH, Littlewood JE, Pólya G (1934) Inequalities. Cambridge
...
\item
  Landrum G (2024) RDKit: open-source cheminformatics software.
  https://www.rdkit.org. Accessed 15 Mar 2026
\item
  Kuhn S, Schlörer NE (2015) Facilitating quality control for spectra
