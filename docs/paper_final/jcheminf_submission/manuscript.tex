...
\title{Learning $^1$H and $^{13}$C NMR chemical shifts jointly from unassigned 2-D HSQC peak sets}
\author{}
\date{}
...
\textbf{Background.} Nuclear magnetic resonance (NMR) chemical-shift
...
\textbf{Results.} We train a four-layer graph isomorphism network on a
...
(i) \textbf{Bonferroni-corrected split-conformal calibration} delivers a
rigorous \(94.8\%\) molecule-level coverage guarantee at
\(\alpha = 0.05\), alongside per-atom coverage of \(95.2\%\) / \(96.7\%\);
(ii) a \textbf{causal \(^{1}\)H-zeroing audit}, in which the \(^{1}\)H
...
\textbf{sliced sort-match loss} reduces the 2-D optimal-transport
...
batchable through \texttt{torch.sort} without a custom autograd hack;
...
\textbf{Conclusions.} Complementary to the feasibility established
...
in this paper uses HSQC multisets \textbf{derived from NMRShiftDB2
...
artefact map are at
\texttt{https://github.com/dongzhaohe321418-lab/nmr-ssl} (release
\texttt{v2.0-jin-revision}).

\textbf{Keywords:} NMR chemical shift prediction; semi-supervised learning;
...
\textbf{First --- a formal molecule-level coverage guarantee.} We wrap the
...
\textbf{Second --- a causal audit of the supervision pathway.} A shared
...
\textbf{Third --- a sliced-Wasserstein alternative to exact Hungarian
...
Supplementary Information \textbf{S8}, so that the core methodological
result of this paper is self-contained. The sliced construction is
permutation-invariant, differentiable through \texttt{torch.sort}, and
...
\textbf{Fourth --- an explicit dereplication experiment.} We evaluate the
...
\textbf{Boundary of the claim.} All experiments in this paper use HSQC
multisets \textbf{derived from NMRShiftDB2 atom-assigned spectra}, not
...
The rest of the paper is organized as follows. \emph{Methods} writes out
the sliced sort-match loss and its theoretical motivation, pointing
to Supplementary \textbf{S8} for the full statement and proof of the
underlying 1-D sort-match theorem. \emph{Experimental details} covers
dataset construction, model architecture, training protocol,
split-conformal calibration, and the structure-verification
procedure. \emph{Results} reports the main label-efficiency numbers, the
...
analysis. \emph{Discussion} covers honest limitations and the path to
real literature HSQC data. \emph{Conclusions} summarizes the findings
...
Supplementary \textbf{S8}, so that the methodological backbone of this
...
directions gives the \textbf{sliced sort-match loss},
...
\texttt{torch.sort}, and computed in \(O(Kn\log n)\) per molecule. It is a
consistent estimator of \(SW_{2}^{2}\) as \(K \to \infty\), and satisfies
\(SW_{2}^{2} \leq W_{2}^{2}\) by the classical inequality.

\textbf{Numerical audit.} On 20 random two-dimensional point-set pairs drawn
...
two-dimensional bound. Implementation: \texttt{src/nmr2d/losses\_2d.py}.
Verification test: \texttt{tests/test\_theorem\_2d.py}.
...
cost (see \emph{Results: Hyperparameter sensitivity}). We report both
...
We parse the NMRShiftDB2 {[}9{]} dump \texttt{nmrshiftdb2withsignals.sd}
...
through \texttt{torch.sort} is standard. Code: \texttt{src/nmr2d/losses\_2d.py}. The
...
per candidate during evaluation. Code: \texttt{src/nmr2d/conformal.py},
\texttt{experiments/compute\_bonferroni\_conformal.py}.
...
\texttt{src/nmr2d/conformal.py::ConformalCalibrator.structure\_verification\_score}.

\textbf{Scope caveat.} The protocol above is \textbf{training-unassigned but
...
one observed HSQC) and does not support \emph{de novo} assignment of a
...
seeds 0, 1, 2, using the training protocol in \emph{Experimental details}.
The dual-head
...
\textbf{Table 1.} Main test-set MAE on NMRShiftDB2 (3 seeds, mean \(\pm\) std).
...
\textbf{2-D SSL} (\(10\%\) labelled, \(\lambda=2\), \(K=16\)) & \(\mathbf{4.53 \pm 0.11}\) & \(\mathbf{0.35 \pm 0.02}\) \\
\textbf{2-D SSL + full \(^{13}\)C} (combined) & \(\mathbf{3.23 \pm 0.10}\) & \(\mathbf{0.30 \pm 0.03}\) \\
\end{longtable}
}
...
contribution of this paper is explicitly \emph{not} in the headline MAE
--- it is in the loss family, the causal audit, the calibration, and
the dereplication experiment (see \emph{Related work and contributions
of this paper}).

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Main result. (a) \^{}\{13\}C test MAE across five training variants: supervised-1-D baseline (grey), 1-D sort-match SSL (blue), 2-D SSL at earlier K=8 working point (orange), 2-D SSL at K=16 low-label headline (purple), 2-D SSL combined with full \^{}\{13\}C supervision (green). (b) \^{}\{1\}H test MAE for the same five variants. Error bars are one standard deviation over 3 seeds. The K=16 low-label headline closes the \^{}\{13\}C gap to the 1-D SSL baseline; the combined variant delivers a further 1.3 ppm improvement.}]{figures/fig_v4_headline.pdf}}
\caption{Main result. (a) \(^{13}\)C test MAE across five training
...
\textbf{Table 2.} Causal audit. The \(^{1}\)H error collapses by more than a
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Causal audit. (a) \^{}\{13\}C test MAE is essentially insensitive to zeroing the HSQC \^{}\{1\}H coordinate, confirming that the \^{}\{13\}C head is trained by its own supervised loss. (b) \^{}\{1\}H test MAE collapses by more than a factor of ten when the HSQC \^{}\{1\}H coordinate is zeroed. The collapse rules out encoder leakage from the \^{}\{13\}C supervised loss as the training signal for the \^{}\{1\}H head, and localizes the training signal at the HSQC target itself.}]{figures/fig_h_zero.pdf}}
\caption{Causal audit. (a) \(^{13}\)C test MAE is essentially insensitive to
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Label-efficiency curve. (a) \^{}\{13\}C MAE as a function of the \^{}\{13\}C-labelled fraction. Supervised-1-D (blue dashed) is useless below 10\textbackslash\% labels; 2-D SSL (green solid) remains within 6 ppm across the entire range and tracks supervised-1-D above 50\textbackslash\% labels. (b) \^{}\{1\}H MAE. Supervised-1-D is pinned at the random floor (\textbackslash sim\textbackslash!3 ppm) because its \^{}\{1\}H head receives no training signal; 2-D SSL delivers sub-0.45 ppm \^{}\{1\}H at every label fraction.}]{figures/fig_label_sweep.pdf}}
\caption{Label-efficiency curve. (a) \(^{13}\)C MAE as a function
...
\textbf{Table 3.} Structure-verification discrimination for the three negative
...
\bottomrule\noalign{}
\endlastfoot
\textbf{Constitutional isomer (headline)} & \(77.0\%\) (n=74) & \(21.7\%\) & \(\mathbf{3.5\times}\) \\
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.92\linewidth]{figures/fig_wrong_struct_v4.pdf}
...
\textbf{Table 4.} Random vs scaffold-OOD generalization. Scaffold-OOD uses
...
produce. Our \texttt{src/nmr2d/realistic\_hsqc.py} pipeline applies four
...
\textbf{Table 5.} Realistic HSQC degradation stress test (3 seeds per row).
...
\textbf{Table 6.} 3-seed \(K\)-sweep and \(\lambda\)-sweep (20 epochs each).
...
\textbf{16} & \(\mathbf{5.32 \pm 0.30}\) & \(\mathbf{0.55 \pm 0.03}\) \\
...
\textbf{2.0} & \(\mathbf{4.72 \pm 0.17}\) & \(\mathbf{0.37 \pm 0.03}\) \\
\end{longtable}
}
...
\textbf{Table 7.} \(^{13}\)C test MAE decomposed by carbon type.
...
Split-conformal calibration on the validation split (\emph{Experimental details: Split-conformal calibration})
...
\textbf{Table 8.} Conformal coverage at \(\alpha = 0.05\) (validation
...
\textbf{Bonferroni molecule-adaptive} & --- & --- & \(\mathbf{94.8\%}\) (147 / 155) \\
\end{longtable}
}
...
\(^{1}\)H labels, and the causal audit (\emph{Causal audit}) confirms the
...
noise at eight times lower cost (see \emph{Hyperparameter sensitivity}).
The \(^{1}\)H and
...
\item
  \textbf{Synthetic HSQC supervision, not scraped literature peak lists.} We
  ...
  experimental noise characteristics. \emph{Robustness to realistic HSQC
  degradation} establishes
  robustness to a model of realistic degradation but does not
  substitute for an actual scraped-literature evaluation. \textbf{This is
  the single largest outstanding gap.}
\item
  \textbf{Absolute accuracy below published baselines on both
  ...
  scale-driven gap.
\item
  \textbf{Diastereotopic \(^{1}\)H averaging excludes stereochemistry
...
\item
  \textbf{Structure verification is a ranker, not a classifier.} The
...
\item
  \textbf{Bonferroni-corrected molecule-level intervals are too wide for
...
HSQC multisets \textbf{derived from NMRShiftDB2 atom-assigned spectra}
...
result, in the order stated in \emph{Background: Related work and
...
tractable at small-lab scale and batchable through \texttt{torch.sort}.
...
(\texttt{nmrshiftdb2withsignals.sd}), release 2026-03-15, distributed under
...
dataset construction filters in \texttt{src/nmr2d/data\_2d.py} applied to
seed-0 random split indices from \texttt{experiments/run\_2d\_experiment.py}.
...
the public repository listed under \emph{Code availability}, at the
...
\texttt{https://github.com/dongzhaohe321418-lab/nmr-ssl} under the MIT
license (software) and CC-BY 4.0 (documentation and figures). The
\textbf{submission-frozen version} is archived as the git tag
\texttt{v2.0-jin-revision} and the corresponding GitHub release. A
mapping from each table and figure in this paper to its exact
generation script and result JSON is provided in the file
\texttt{docs/paper\_final/ARTIFACT\_MAP.md} at that tag, so each headline
...
repository's \texttt{pyproject.toml}. The end-to-end training pipeline is
launched by
\texttt{python3\ experiments/run\_option\_b\_master.py\ -\/-seeds\ 0\ 1\ 2};
the causal \(^{1}\)H-zeroing audit by
\texttt{python3\ experiments/run\_h\_zero\_ablation.py};
and the Bonferroni-corrected conformal calibration by
\texttt{python3\ experiments/compute\_bonferroni\_conformal.py}. A Zenodo
...
\texttt{v2.0-jin-revision} via the artefact map (see \emph{Code availability}).
...
\textbf{Table S1.} Per-seed test MAE for the low-label 2-D SSL headline
...
1 & \(4.451\) & \(0.351\) \\
2 & \(4.464\) & \(0.376\) \\
\textbf{Mean \(\pm\) std} & \textbf{\(4.535 \pm 0.110\)} & \textbf{\(0.353 \pm 0.018\)} \\
\end{longtable}
}

\subsection*{S2. Per-seed results for the combined full-supervision variant}

\textbf{Table S2.} Per-seed test MAE for the combined recipe (full
...
1 & \(3.104\) & \(0.350\) \\
2 & \(3.252\) & \(0.275\) \\
\textbf{Mean \(\pm\) std} & \textbf{\(3.231 \pm 0.097\)} & \textbf{\(0.304 \pm 0.033\)} \\
\end{longtable}
}
...
histogram loss is \textbf{neutral} at the default weight
\(\lambda_{\text{mul}} = 1\):

\textbf{Table S3.} Multiplicity-augmented loss.
...
\texttt{experiments/run\_multiplicity\_loss.py}.
...
\textbf{Table S4.} Stop-gradient control (3 seeds, 30 epochs).
...
\textbf{disproves} the leakage hypothesis: if leakage were the mechanism,
...
\textbf{Table S5.} Pretrain-then-finetune.
...
\textbf{Combined (no phase switch)} & --- & \textbf{\(3.23 \pm 0.10\)} & \textbf{\(0.30 \pm 0.03\)} \\
\end{longtable}
}
...
\texttt{experiments/results\_2d/stereo\_demo.json}.

\subsection*{S7. Loss-code audit}

An independent code review verified that the sliced sort-match
implementation in \texttt{src/nmr2d/losses\_2d.py} faithfully matches the
the \emph{Methods} construction, that the axis-aligned \(K=2\) variant corresponds
to the two canonical coordinate projections, that the ratio check in
\emph{Sliced approximation} reports mean \(0.414\) with the expected range for
dimension-2 sliced Wasserstein, and that the gradient is finite and
non-NaN end-to-end. The verification test is
\texttt{tests/test\_theorem\_2d.py}; it reproduces the mean ratio and
...
\emph{Methods: Set-valued NMR supervision} and cited in the second half
of \emph{Background: Related work and contributions} (third contribution).
...
\textbf{Setup.} Fix an integer \(n \geq 1\). Let
...
\textbf{Theorem S8.1 (1-D sort-match is optimal bipartite matching).} For
...
\emph{Proof.} Expand the squared error:
\[
...
\(\sum_{i} x_{i}\,y_{\pi(i)}\) is \textbf{maximized} over
...
\textbf{Remark S8.2 (ties and multisets).} The theorem as stated holds
...
\textbf{Remark S8.3 (from \(n^{3}\) Hungarian to \(n\log n\) sort).} The
...
\textbf{Remark S8.4 (why the 2-D problem does not reduce to a single sort).}
...
our training loss \(\mathcal{L}_{\mathrm{SSW}}\) in \emph{Methods:
