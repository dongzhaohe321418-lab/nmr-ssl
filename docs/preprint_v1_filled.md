...
| Naive SSL (wrong assignment) | 11.420 ± 0.482 | +208% (worse) |
| **Sort-match SSL (ours)** | **3.405 ± 0.102** | **−8.2% (better)** |

Three observations.

First, **sort-match SSL strictly beats supervised on every seed** (paired on the same splits), and the mean difference (0.30 ppm, 8.2% relative) exceeds 2.4× the combined standard deviation — non-overlapping error bars. The improvement is robust to seed choice.

Second, **naive SSL is nearly 3× *worse* than supervised**, not better. Dumping unassigned unlabeled data into the training loop with an arbitrary assignment actively fights the correct labeled loss; the validation trajectory is chaotic (Fig. 2a). This is the negative control that proves the *loss function* — not merely "using more data" — is what drives the sort-match gain.
...
A semi-supervised method's value lies in the low-label regime. We swept labeled fraction over $\{0.02, 0.05, 0.10, 0.20\}$ with the same 17,305-molecule dataset, 20 epochs, $n=2$ seeds per fraction, and otherwise identical configuration. Figure 3a shows the result: sort-match SSL's advantage over supervised grows monotonically as labeled data becomes scarce, from 2.8% relative at 20% labeled (3.504 → 3.403 ppm) to 37.4% relative at 2% labeled (5.784 → 3.622 ppm). Supervised MAE climbs from 3.50 to 5.78 ppm as labels vanish; sort-match MAE stays nearly flat in the 3.40–3.85 range. This is the expected signature of an effective SSL method.
...
|---|---|
| None (clean) | 3.337 ± 0.043 |
| Gaussian shift noise σ = 1 ppm | 3.345 ± 0.026 |
| 15% of peaks dropped | 3.805 ± 0.133 |
| Combined: 1 ppm noise + 10% drop + 10% spurious | 3.822 ± 0.122 |
