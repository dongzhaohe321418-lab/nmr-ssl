...
def main():
    ...
    ax = axes[0]
    bars = ax.bar(x, c_vals, color=colors, edgecolor="black", linewidth=0.6, width=0.6)
    ...
    ax.set_ylim(0, max(c_vals) * 1.2)

    ax = axes[1]
    bars = ax.bar(x, h_vals, color=colors, edgecolor="black", linewidth=0.6, width=0.6)
    for b, v in zip(bars, h_vals):
        ax.text(b.get_x() + b.get_width() / 2, v + 0.1, f"{v:.2f}", ha="center", va="bottom", fontsize=8)
    ax.set_xticks(x)
    ax.set_xticklabels(labels, fontsize=8)
    ax.set_ylabel("$^{1}$H test MAE (ppm)")
    ax.set_title("(b) $^{1}$H — collapses ~10× when HSQC $^{1}$H zeroed")
    ax.set_ylim(0, max(h_vals) * 1.2)

    # Annotate the arrow showing collapse
    arrow_x = 2.3
    ax.annotate("", xy=(2, baseline_h + 0.15), xytext=(1.5, hz_h - 0.2),
                arrowprops=dict(arrowstyle="->", color=RED, lw=1.5))
    ax.text(1.7, (hz_h + baseline_h) / 2, f"{hz_h / baseline_h:.1f}×", color=RED, fontsize=10, fontweight="bold")
